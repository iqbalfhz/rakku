...
class BalanceOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Total saldo', $this->formatRupiah((float) $book->accounts()->sum('current_balance')))
                ->description($book->accounts()->count().' akun')
                ->descriptionIcon(Heroicon::OutlinedWallet)
                ->color('primary'),
            Stat::make('Pemasukan bulan ini', $this->formatRupiah($totals['income']))
                ->descriptionIcon(Heroicon::OutlinedArrowDownLeft)
                ->description(today()->translatedFormat('F Y'))
                ->color('success'),
            Stat::make('Pengeluaran bulan ini', $this->formatRupiah($totals['expense']))
