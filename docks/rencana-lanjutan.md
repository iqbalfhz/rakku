...
| 10 | **Backup off-site** — *sisi kode selesai 25 Sep 2026, tinggal setelan* | `league/flysystem-aws-s3-v3` sudah terpasang, jadi `BACKUP_OFFSITE_DISK=s3` sekarang bisa dipakai. Salinannya diverifikasi (ada dan seukuran) dan setiap kegagalan dikirim ke admin lewat [`BackupFailed`](../app/Notifications/BackupFailed.php) — sebelumnya salinan yang ditolak penyimpanan tetap dilaporkan berhasil. **Yang tersisa:** (a) buat bucket + isi `AWS_*` di Coolify; (b) arahkan juga **backup database milik Coolify** ke bucket yang sama — itu bagian terbesar datanya dan sekarang masih satu mesin dengan aslinya; (c) sekali saja, coba pulihkan dari arsipnya, karena cadangan yang belum pernah dipulihkan belum terbukti. |
