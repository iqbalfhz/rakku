...
| 10 | ~~**Backup off-site**~~ — **selesai 25 Sep 2026** | `league/flysystem-aws-s3-v3` sudah terpasang, jadi `BACKUP_OFFSITE_DISK=s3` sekarang bisa dipakai. Salinannya diverifikasi (ada dan seukuran) dan setiap kegagalan dikirim ke admin lewat [`BackupFailed`](../app/Notifications/BackupFailed.php) — sebelumnya salinan yang ditolak penyimpanan tetap dilaporkan berhasil. Keduanya sudah berjalan di produksi ke bucket Cloudflare R2 `rakku-backup`: arsip file unggahan lewat perintah ini, dan dump MySQL lewat backup terjadwal Coolify (*Keep local backup* + S3, retensi 14). **Satu-satunya yang belum:** memulihkan sungguhan dari salah satu cadangan itu — ke database sekali pakai, bukan ke produksi. Sampai itu dilakukan, yang terbukti baru bahwa berkasnya ada dan isinya terbaca, belum bahwa ia bisa mengembalikan aplikasi. |
