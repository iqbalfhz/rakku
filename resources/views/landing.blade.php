<x-layouts.public
    title="RakKu — Pembukuan rapi untuk usaha kecil"
    description="Catat pemasukan dan pengeluaran, pantau utang-piutang, dan kirim invoice ber-PDF. RakKu adalah buku kas digital untuk usaha kecil dan keuangan pribadi."
    :login-url="$loginUrl"
...
>
    ...
    {{-- Tanya jawab, memakai elemen bawaan browser supaya tetap ringan --}}
    <section id="tanya-jawab" class="border-b border-rule bg-paper-deep">
        <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 lg:py-28">
            <div class="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
                <h2 class="font-serif text-4xl leading-tight sm:text-5xl">
                    Yang biasa<br>ditanyakan
                </h2>

                <div>
                    @foreach ($questions as $question)
                        <details class="group rule-row py-5">
                            <summary class="flex cursor-pointer items-center justify-between gap-4 font-medium marker:content-['']">
                                {{ $question['question'] }}
                                <span class="shrink-0 text-xl text-ink-soft transition group-open:rotate-45">+</span>
                            </summary>
                            <p class="mt-3 leading-relaxed text-ink-soft">{{ $question['answer'] }}</p>
                        </details>
                    @endforeach
                </div>
            </div>
